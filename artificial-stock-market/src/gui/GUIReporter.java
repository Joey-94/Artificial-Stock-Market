...
public class GUIReporter implements Steppable {
	...
	public ArrayList<XYSeries> informSeries = new ArrayList<XYSeries>();//by WEI
	
	public ArrayList<XYSeries> tPriceSeries = new ArrayList<XYSeries>();
	...
	public ArrayList<Vector<Double>> informMemory = new ArrayList<Vector<Double>>();
	
	public ArrayList<Vector<Double>> tPriceMemory = new ArrayList<Vector<Double>>();
	...
	public GUIReporter(FinancialModelWithUI contModelWithUI) {
		...
		for (int a = 0; a < myModel.parameterMap.get("numAssets"); a++) {
		    ...
		    informSeries.add(new XYSeries("FundamentalValue for asset " + a));
		    tPriceSeries.add(new XYSeries("trade price for asset " + a));
			...
			informSeries.get(a).setMaximumItemCount(NumViewable);
			tPriceSeries.get(a).setMaximumItemCount(NumViewable);
			...
			informMemory.add(new Vector<Double>());
			tPriceMemory.add(new Vector<Double>());
	...
	public void step(SimState state) {
		...
		if (state.schedule.getTime() >= nextUpdate) {
			...
			for (int a = 0; a < myModel.parameterMap.get("numAssets"); a++) {
				...
				//informMemory.get(a).add((myModel.myMarket.getInformValueForAsset(a)));
				informMemory.get(a).add((myModel.myAsset.getCommonValue()));
				tPriceMemory.get(a).add((myModel.myMarket.getTPriceForAsset(a)));
	...
	public void setSeries() {
		...
		for (int a = 0; a < myModel.parameterMap.get("numAssets"); a++) {
			for (int i = start; i < stop; i++) {
				...
				informSeries.get(a).add(i, informMemory.get(a).get(i));
				tPriceSeries.get(a).add(i, tPriceMemory.get(a).get(i));
